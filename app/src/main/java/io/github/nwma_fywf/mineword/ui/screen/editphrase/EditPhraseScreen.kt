...
fun EditPhraseScreen(
    ...
    val canSave = phraseText.isNotBlank()
                ...
                actions = {
                    TextButton(
                        onClick = {
                            scope.launch {
                                phrase?.let { p ->
                                    if (viewModel.checkDuplicate(phraseText.trim(), p.id)) {
                                        duplicateWarning = "词组 \"${phraseText.trim()}\" 已存在"
                                    } else {
                                        viewModel.updatePhrase(
                                            phrase = p,
                                            newPhrase = phraseText.trim(),
